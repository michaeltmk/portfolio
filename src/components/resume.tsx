...
import { useResumeInfo } from '@/lib/portfolio-context';

export function Resume() {
  // Load resume details from context
  const resumeDetails = useResumeInfo();

  const handleDownload = () => {
    // Create a link element
    const link = document.createElement('a');
    link.href = resumeDetails.download_url;
    link.download = resumeDetails.download_url.split('/').pop() || 'resume.pdf';
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="mt-1 flex text-xs text-muted-foreground">
                <span>{resumeDetails.file_type}</span>
                <span className="mx-2">•</span>
                <span>Updated {resumeDetails.last_updated}</span>
                <span className="mx-2">•</span>
                <span>{resumeDetails.file_size}</span>
              </div>
            </div>
